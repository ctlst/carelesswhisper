# CarelessWhisper Ideas

## Voice Wake-Up While Disabled

Current behavior: voice commands are processed only after CarelessWhisper records and transcribes an utterance. If the user says `stop listening`, the app disables recording, so it cannot hear `start listening` afterward.
...
- Require explicit phrases such as `careless whisper start listening` to avoid accidental reactivation.
...
Possible improvement: keep a small native command listener alive while Enabled is on.
